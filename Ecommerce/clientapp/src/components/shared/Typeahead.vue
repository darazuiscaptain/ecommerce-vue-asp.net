<template>
    <form-input-base :label="label">
        <div class="typeahead">
            <input ref="input" :value="value" :name="name" :class="classes" @blur="blur" @focus="focus"
                @keydown.enter.prevent="enter" @keydown.down.prevent="down" @keydown.up.prevent="up" @input="input" />
            ...
            <b-list-group v-if="open" class="suggestions">
                <b-list-group-item v-for="(suggestion, index) in matches" :key="index" :active="index === current"
                    @mousedown.prevent="click(index)" button>
                    {{ suggestion }}
                </b-list-group-item>
            </b-list-group>
        </div>
